...
class AlertEngine:
    ...
    DEFAULT_RULES = [
        ...
        AlertRule(
            "hashrate_drop", "hashrate_drop_pct", ">", 30.0, "WARN", "hashrate_drop"
        ),
        # ── Reject / Stale ──
    ...
    def _load_rules(self, tenant_id: str = "") -> List[AlertRule]:
        try:
            ...
            if tenant_id:
                c.execute(
                    "SELECT * FROM alert_rules WHERE enabled=1 AND tenant_id=?",
                    (tenant_id,),
                )
            else:
    ...
    def evaluate(
        self, devices: List[Device], pool: Optional[Dict[str, Any]] = None
    ) -> List[Alert]:
