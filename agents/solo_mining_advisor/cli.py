        ...
        def completer(text, state):
            ...
            if not tokens or len(tokens) == 1:
                if tokens and line.endswith(" ") and tokens[0] in _COMPLETIONS:
                    ...
                    prefix = (
                        tokens[0]
                        if (tokens and line.endswith(" ") and not text)
                        else text
                    )
                    options = [
                        c for c in _COMPLETIONS["__commands__"] if c.startswith(prefix)
                    ]
            ...
            if verb in _COMPLETIONS:
                ...
                available = [
                    f for f in flags if f not in already_used and f.startswith(text)
                ]
...
def cmd_help():
    ...
    print(
        "       Show full mining dashboard: difficulty, BTC price, worker stats, 24h probability"
    )
    print()
    ...
    print()
    print(
        f"  {C.GREEN}calc --hashrate <value> --duration <h>{C.RST} {C.MUTED}[--difficulty <d>] [--json]{C.RST}"
    )
    ...
    print()
    print(
        f"  {C.GREEN}compare --budget <btc> --duration <h>{C.RST} {C.MUTED}[--braiins <p>] [--mrr <p>] [--json]{C.RST}"
    )
...
def cmd_status(json_mode=False):
    ...
    print(
        f"{C.CYAN}{C.BOLD}║{C.RST}          {C.BOLD}CYPHER MINING STATUS{C.RST}              {C.CYAN}{C.BOLD}║{C.RST}"
    )
    print(f"{C.CYAN}{C.BOLD}╚══════════════════════════════════════════════╝{C.RST}")
    print()
    ...
    if difficulty:
        print(
            f"  {C.BOLD}Difficulty{C.RST}        {C.WHITE}{difficulty:,.0f}{C.RST}  ({C.MUTED}{_fmt_diff_human(difficulty)}{C.RST})"
        )
        print(
            f"  {C.BOLD}Source{C.RST}           {C.MUTED}{diff_result.get('source', 'agent')}{C.RST}"
        )
    else:
    ...
    if prices:
        parts = []
        if prices.get("usd"):
            parts.append(f"{C.WHITE}${prices['usd']:,.0f}{C.RST}")
        if prices.get("brl"):
            parts.append(f"{C.WHITE}R${prices['brl']:,.0f}{C.RST}")
    ...
    if worker_hr:
        print(
            f"  {C.BOLD}Hashrate{C.RST}        {C.WHITE}{_fmt_hashrate_human(worker_hr)}{C.RST}"
        )
    else:
        print(
            f"  {C.BOLD}Hashrate{C.RST}        {C.MUTED}no worker data from pool{C.RST}"
        )
    ...
    print(
        f"  {C.BOLD}Status{C.RST}          {status_color}{worker_status.upper()}{C.RST}"
    )

    uptime = pool_result.get("worker_uptime")
    if uptime:
        print(
            f"  {C.BOLD}Uptime{C.RST}          {C.WHITE}{_fmt_uptime_human(uptime)}{C.RST}"
        )
    ...
    if pool_hr:
        print(
            f"  {C.BOLD}Pool Hashrate{C.RST}   {C.WHITE}{_fmt_hashrate_human(pool_hr)}{C.RST}"
        )
    print(
        f"  {C.BOLD}Workers{C.RST}         {C.WHITE}{pool_result.get('pool_workers', '—')}{C.RST} / {C.WHITE}{pool_result.get('pool_users', '—')} users{C.RST}"
    )
    print(
        f"  {C.BOLD}Data Status{C.RST}     {C.MUTED}{pool_result.get('pool_status', 'unknown')}{C.RST}"
    )
    print()
    ...
    if prob_data:
        ...
        if not worker_hr and not difficulty:
            print(
                C.warn(
                    "Block probability skipped — both hashrate and difficulty are missing."
                )
            )
        elif not worker_hr:
            print(
                C.warn(
                    "Block probability skipped — no worker hashrate from parasite.space."
                )
            )
        elif not difficulty:
...
def cmd_network(json_mode=False):
    ...
    if diff_result.get("difficulty"):
        ...
        print(
            f"  {C.BOLD}formatted{C.RST}......... {C.WHITE}{_fmt_diff_human(diff)}{C.RST}"
        )
        print(
            f"  {C.BOLD}source{C.RST}............ {C.MUTED}{diff_result.get('source', 'agent tool')}{C.RST}"
        )
    else:
    ...
    if prices:
        if prices.get("usd"):
            print(
                f"  {C.BOLD}btc/usd{C.RST}........... {C.WHITE}${prices['usd']:,.0f}{C.RST}"
            )
        if prices.get("brl"):
            print(
                f"  {C.BOLD}btc/brl{C.RST}........... {C.WHITE}R${prices['brl']:,.0f}{C.RST}"
            )
        if prices.get("eur"):
            print(
                f"  {C.BOLD}btc/eur{C.RST}........... {C.WHITE}€{prices['eur']:,.0f}{C.RST}"
            )
        print(
            f"  {C.BOLD}source{C.RST}............ {C.MUTED}{price_result.get('source', 'coingecko.com')}{C.RST}"
        )
    else:
...
def cmd_calc(args, json_mode=False):
    ...
    if not hashrate or not duration:
        print(
            C.err(
                "Missing required flags. Usage: calc --hashrate <value> --duration <h> [--difficulty <d>]"
            )
        )
    ...
    if not difficulty:
        ...
        if diff_result.get("difficulty"):
            ...
            if not json_mode:
                print(
                    C.ok(
                        f"Using live difficulty: {_fmt_diff_human(difficulty)} ({diff_result.get('source', 'agent')})"
                    )
                )
        else:
            if not json_mode:
    ...
    print(
        f"  {C.BOLD}hashrate{C.RST}........... {C.WHITE}{hashrate}{C.RST} ({hashrate_hs:,.0f} H/s)"
    )
    print(
        f"  {C.BOLD}duration{C.RST}............ {C.WHITE}{duration}h ({duration / 24:.2f} days){C.RST}"
    )
    print(
        f"  {C.BOLD}difficulty{C.RST}.......... {C.WHITE}{difficulty:,.0f}{C.RST} ({_fmt_diff_human(difficulty)})"
    )

    print()
    print(C.header("Block Discovery"))
    print(
        f"  {C.BOLD}Hashes per block{C.RST}.... {C.WHITE}{prob['hashes_per_block']:,.0f}{C.RST}"
    )
    print(
        f"  {C.BOLD}Block rate{C.RST}........... {C.MUTED}{prob['block_rate_per_sec']:.6e}{C.RST} blocks/s"
    )
    print(
...
def cmd_compare(args, json_mode=False):
    ...
    if not budget or not duration:
        print(
            C.err(
                "Missing required flags. Usage: compare --budget <btc> --duration <h> [--braiins <p>] [--mrr <p>]"
            )
        )
    ...
    if braiins_price is None:
        ...
        if result.get("price_btc_per_ph_day") is not None:
            ...
            if not json_mode:
                print(
                    C.ok(
                        f"Braiins: {braiins_price:.8f} BTC/PH/day ({result.get('available_asks', '?')} asks)"
                    )
                )
        elif not json_mode:
    ...
    if mrr_price is None:
        ...
        if result.get("price_btc_per_ph_day") is not None:
            ...
            if not json_mode:
                print(
                    C.ok(
                        f"MRR: {mrr_price:.8f} BTC/PH/day ({result.get('total_listings', '?')} listings)"
                    )
                )
        elif not json_mode:
    ...
    print(
        C.ok(
            f"Budget: {budget} BTC | Duration: {duration}h | Difficulty: {_fmt_diff_human(difficulty)}"
        )
    )
    ...
    if not results:
        ...
        if mrr_price is None and braiins_price is None:
            print(
                C.warn(
                    "Tip: set MRR_API_KEY/MRR_API_SECRET env vars for live MRR pricing"
                )
            )
        return
    ...
    if len(results) == 1 and results[0].get("api_status") == "error":
        ...
        print(
            C.warn(
                "Provide prices manually: --braiins <price> --mrr <price> (BTC/PH/day)"
            )
        )
        return
    ...
    if len(results) == 1 and "MRR" not in results[0]["platform"] and mrr_price is None:
        print(
            C.warn(
                "Note: MRR was skipped (no credentials or API error). Set MRR_API_KEY/MRR_API_SECRET to include MRR in comparison."
            )
        )
    if results[0]["ev_btc"] < 0:
        ...
        print(
            C.ok(
                f"Best option: {results[0]['platform']} (EV={results[0]['ev_btc']:+.6f} BTC)"
            )
        )
...
def cmd_ask(args):
    ...
    if not query:
        ...
        print(
            C.hint(
                "Examples: ask qual a chance de achar bloco? | ask what's the difficulty?"
            )
        )
        return
    ...
    if any(k in q for k in compare_kw):
        ...
        dur_match = re.search(
            r"(\d+\.?\d*)\s*(h|hour|hr|hours|horas|hora|d|day|days|dia|dias)", q
        )
    ...
    if any(k in q for k in calc_kw):
        ...
        hr_match = re.search(
            r"(\d+\.?\d*)\s*(th/s|ph/s|eh/s|gh/s|mh/s|th|ph|eh|gh|mh)", q, re.IGNORECASE
        )
        dur_match = re.search(
            r"(\d+\.?\d*)\s*(h|hour|hr|hours|horas|hora|d|day|days|dia|dias|w|week|weeks|semana|semanas)",
            q,
        )
    ...
    print(
        f"  {C.GREEN}•{C.RST} Digite '{C.GREEN}network{C.RST}' — dificuldade e preço do BTC agora"
    )
    print(
        f"  {C.GREEN}•{C.RST} Digite '{C.GREEN}status{C.RST}' — resumo completo da sua mineração"
    )
    print(
        f"  {C.GREEN}•{C.RST} Digite '{C.GREEN}calc --hashrate 225TH --duration 24h{C.RST}' — chance de achar bloco"
    )
    print(
        f"  {C.GREEN}•{C.RST} Digite '{C.GREEN}compare --budget 0.01 --duration 24h{C.RST}' — comparar aluguel"
    )
    print(f"  {C.GREEN}•{C.RST} Digite '{C.GREEN}help{C.RST}' — todos os comandos")
    print(
        f"  {C.GREEN}•{C.RST} Ou me pergunte de outro jeito: 'qual a chance de achar bloco com 500th por 7 dias?'"
    )
...
def cmd_chat(args=None):
    ...
    print(
        C.hint(
            "Digite qualquer pergunta em linguagem natural. Ex: 'qual a chance de achar bloco com 500th?'"
        )
    )
    print(
        C.hint(
            f"Digite '{C.GREEN}exit{C.RST}', '{C.GREEN}sair{C.RST}' ou '{C.GREEN}voltar{C.RST}' para retornar ao terminal principal."
        )
    )
    print()
    ...
    while True:
        ...
        if any(k in q for k in compare_kw):
            btc_match = re.search(
                r"(\d+\.?\d*)\s*(btc|sat|sats|bitcoin)", q, re.IGNORECASE
            )
            dur_match = re.search(
                r"(\d+\.?\d*)\s*(h|hour|hr|hours|horas|hora|d|day|days|dia|dias)", q
            )
            if btc_match and dur_match:
                ...
                print(
                    C.warn(
                        "Consegui identificar que é sobre aluguel. Me passe orçamento e duração."
                    )
                )
        ...
        if any(k in q for k in calc_kw):
            hr_match = re.search(
                r"(\d+\.?\d*)\s*(th/s|ph/s|eh/s|gh/s|mh/s|th|ph|eh|gh|mh)",
                q,
                re.IGNORECASE,
            )
            dur_match = re.search(
            ...
            if hr_match and dur_match:
                hashrate = hr_match.group(1) + hr_match.group(2).upper().replace(
                    "/S", ""
                )
                dur_val = float(dur_match.group(1))
                dur_unit = dur_match.group(2).lower()
                ...
                print(
                    C.warn(
                        "Consegui identificar que é sobre mineração. Me passe hashrate e tempo."
                    )
                )
        ...
        if verb in commands:
            ...
            try:
                ...
                if out_file:
                    ...
                    result = _dispatch(
                        verb, args, json_mode, commands, _ARGS_COMMANDS, _JSON_COMMANDS
                    )
