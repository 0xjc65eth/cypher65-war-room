...
_AXEOS_INFO = {
    ...
    "temp": 53.2, "fanRPM": 4600, "fanSpeed": 92, "power": 15.6,
    "coreVoltage": 1201, "frequency": 550, "bestDiff": "8.2T",
    "sharesAccepted": 1450, "sharesRejected": 7, "uptime": 86500,
    "stratumURL": "public-pool.io", "stratumPort": 21496,
    "stratumUser": "bc1qtest.gamma01", "wifiRSSI": -52,
}
...
class _AxeOSHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        # AxeOS restart/identify over HTTP :80 — counter proves the agent
        # actually executed the queued command on the LAN (command fix).
        path = self.path.rstrip("/")
        if self._is_localhost_probe():
            self.send_response(404)
            self.end_headers()
            return
        if path.endswith("/api/system/restart") or path.endswith("/api/system/identify"):
            global _AXEOS_RESTARTS
            _AXEOS_RESTARTS += 1
            body = b"{\"success\": true}"
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
...
def main() -> int:
    ...
    try:
        ...
        if not _wait_for_server(base):
            print(f"  ❌ servidor não subiu em {POLL_WINDOW_S}s — log:\n{_log_tail(server_log)}")
            return 1
        ...
        if server_url != base:
            ...
            return 1
        print("  3. agente token  ✓  tenant=%s · server_url=%s"
        ...
        while time.time() < deadline:
            ...
            time.sleep(2)
        if len(devices) < 2:
            print("  ❌ devices não registrados no tempo — log do agente:\n"
                  + _log_tail(agent_log))
            print("     log do servidor (últimas linhas):\n" + _log_tail(server_log))
            return 1
        bad = [d for d in devices
               if d.get("status") != "ONLINE"
               or int(d.get("hashrate_hs") or 0) <= 0]
        if bad:
            print(f"  ❌ devices com status/hashrate errados: {json.dumps(bad, default=str)}")
            return 1
        ...
        if not all(bool(d.get("agent_managed")) for d in devices):
            print("  ❌ nem todo device está agent_managed: "
                  + json.dumps(devices, default=str))
            return 1
        print("  5. /api/axe-fleet/devices ✓  %d device(s) ONLINE"
              % len(devices))
        ...
        while time.time() < deadline:
            ...
            time.sleep(2)
        if len(fleet) < 2:
            print("  ❌ axe_fleet vazio/parcial no /api/snapshot — log do agente:\n"
                  + _log_tail(agent_log))
            return 1
        print("  6. /api/snapshot    ✓  axe_fleet com %d device(s) e telemetria"
              % len(fleet))
        ...
        cgminer_dev = next(d for d in devices if d.get("model", "").lower().startswith("antminer"))
        ...
        code, resp = _http_json(
            "POST", base + f"/api/axe-fleet/devices/{cgminer_dev['id']}/restart",
        ...
        code, resp = _http_json(
            "POST", base + f"/api/axe-fleet/devices/{axeos_dev['id']}/restart",
            payload={}, token=access)
        if code not in (200, 201) or not resp.get("queued"):
        ...
        if _AXEOS_RESTARTS < 1:
            print("  ❌ AxeOS mock nunca recebeu o restart (agente não executou):"
                  "\n" + _log_tail(agent_log))
            return 1
        if _CGMINER_RESTARTS < 1:
            print("  ❌ cgminer mock nunca recebeu o restart (agente não executou):"
                  "\n" + _log_tail(agent_log))
            return 1
        print("  7. command round-trip ✓  restart AXEOS(%d) + cgminer(%d) executados na LAN"
        ...
        for d in devices:
            print("%-12s%-20s%-9s%-16d%s" % (
                d.get("ip_address"), (d.get("model") or "?")[:19],
                d.get("status"), int(d.get("hashrate_hs") or 0),
                "1" if d.get("agent_managed") else "0"))
        print("-" * 74)
        print("✅ E2E OK — o fluxo completo do agente local funciona "
              "(token → registro → telemetria → dashboard).")
        return 0
