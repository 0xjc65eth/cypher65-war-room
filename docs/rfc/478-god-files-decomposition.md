...
**Lacuna que PERMANECE aberta (registrada na Issue #557, não tratada lá).** Dois espelhos restam no harness, e eles não são o mesmo caso. (1) **Helpers de auth** — `authBuildHeaders`/`authIsExpired`/`authSessionValid` (Suite 22, ~linhas 2.594–2.631 do harness) são espelhos de código que **já mora** no `38-billing-auth.js`, que o harness **não** carrega; como esse fragmento é lido pelo prefixo **síncrono** do `boot()`, ele é o candidato mais relevante ao mesmo tratamento do #515/#548/#553 (`loadFragment('38-billing-auth.js', …)` com sandbox mínimo). (2) **`acctRankLabels`** (Suite 27) — este é **estrutural**, não dívida de método: a função ainda vive no `40-app-logic.js`, e o harness **não pode** carregar o god file (o `boot();` do topo rodaria). O espelho só sai quando o domínio (R7/R6) for extraído para um fragmento — o que é exatamente o tipo de ganho colateral que a decomposição traz.
...
|---|---|---|
| Frontend | 1, 1b, 2, 2b, 3, 4a, 4b (+ limpeza #525), 5 (#529), 6 (#540), 7 (#542), 7b (#548), 8 (#545), 9 (#551) e 9b (#553) | — (**meta atingida**) |
