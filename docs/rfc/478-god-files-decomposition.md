...
| **9** | Wallet + Support | R3 + R20 | ✅ **#551** (PR #552) — **1.032 linhas** verbatim em **2 blocos**: R3 `44–642` (599) e R20 `2599–3031` (433) → `static/src/37-wallet-support.js` (1.062 com cabeçalho), que entra **ANTES** do 40 | **3.288** | **18** — o maior de todos: listeners de WebLN/wallet, `renderSupportMethods()`/`loadDonations()`/`toggleWalletCTA()` chamados direto no topo, e a região **vendorada do QR** (`(function buildQrMath(){…})()` + 4 × `QrPoly.prototype.*`) |
...
**A meta de ~4.000 linhas é atingida no PR 9** — **executado**: com os PRs 6, 7 e 8 concluídos o god file parava em **4.320**, e o PR 9 (1.032 linhas) o levou a **3.288**, abaixo da meta de ~4.000. **A meta está cumprida.** O residual ainda tem domínios extraíveis — inclusive um PR 10 natural de **Dashboard/`render()`** (R5+R6+R8+R18+R24+R31 ≈ 1.096) — mas eles ficam **fora da meta**; a lista acima é o que fecha o objetivo.
