...
| 4b | Fleet/AXE · AXE Fleet (cards, scan LAN, wizard, agente, remoto) | pendente — `static/src/49-axe-fleet.js` (~1.300 linhas, dentro do guardrail): `renderTailscale`/`fetchTailscale`/`renderRemoteOnboarding`, `fetchAxeFleet`, `scanNetwork`/`renderScanResults`/`renderAxeScanResults`, `renderAxeFleet`/`_renderAxeCard`/`_handleAxeCmdClick`/`openAxeDetail`/`loadDeviceHistoryChart`, `buildCommandCenterRows`, o hash-flow raster (`_lmShareDelta`/`_lmFlowSampleFromDelta`/`_lmFlowDetail`/`_pushLmFlowSample` — prefixo `_lm` mas consumidos **só** pelo `_ccRenderFleet`), `fetchFleetCommandCenter`/`initFleetCommandCenterControls`, o wizard (`gotoAxeWizStep`/`setAxeWizMode`/`resetAxeWizard`/`renderAxeConfirm`/`testAxeConnectivity`/`buildConnectivityReport`), `initAxeFleetControls`, `initAxeAgentPanel` e `addAxeDevice`. É o **único** statement de topo do cluster Fleet: o listener de `#remote-test-btn` |
