...
| **8b** | *(follow-up de cobertura)* Billing/Auth | — (nenhum código movido) | ✅ **#559** — o harness carrega `38-billing-auth.js` via `loadFragment('38-billing-auth.js', …, { window, localStorage })`; o espelho de `authBuildHeaders`/`authIsExpired`/`authSessionValid` sai (**18 asserções mantidas, +7 novas**; JS core **1.428 → 1.435**). O espelho **havia divergido** do fonte: omitia `licenseKey()`, então o header `X-License-Key` (tier PRO) não era coberto em lugar nenhum e a asserção `'no token -> empty headers'` codificava o contrato *do espelho* | **3.292** (inalterado) | — |
| **7b** | *(follow-up de cobertura)* Probability/Block Model | — (nenhum código movido) | ✅ **#548** — harness carrega `42-probability.js` via `loadFragment` (SUITE 33 deixa de espelhar) + `probability-whatif.spec.js` ganha fixture determinístico e comparação numérica | **4.320** (inalterado) | — |
| **9** | Wallet + Support | R3 + R20 | ✅ **#551** (PR #552) — **1.032 linhas** verbatim em **2 blocos**: R3 `44–642` (599) e R20 `2599–3031` (433) → `static/src/37-wallet-support.js` (1.062 com cabeçalho), que entra **ANTES** do 40 | **3.292** | **18** — o maior de todos: listeners de WebLN/wallet, `renderSupportMethods()`/`loadDonations()`/`toggleWalletCTA()` chamados direto no topo, e a região **vendorada do QR** (`(function buildQrMath(){…})()` + 4 × `QrPoly.prototype.*`) |
| **9b** | *(follow-up de cobertura)* Wallet crypto + Support | — (nenhum código movido) | ✅ **#553** (PR #554) — o harness carrega `37-wallet-support.js` via `loadFragment(...)`: o **espelho** de `validateBitcoinAddress`, do núcleo QR (`qrEncode`/`qrSvg`, todo o QR transitivo) e de `walletAddressParts`/`walletHealth` sai (**−441 / +35** linhas no harness). Achado colateral do follow-up: os blocos legados `[btc-valid]`/`[chunk]` imprimiam `FAIL` mas **não incrementavam o contador global** — a suíte terminava com **exit 0 com asserts quebrados** | **3.292** (inalterado) | — |
| **10** | Dashboard / `render()` *(fora da meta)* | R5 + R6 + R8 + R18 + R24 + R31 | ✅ **#561** (PR #564) — **1.082 linhas** verbatim em **6 recortes** (R5 `209–504` 296 · R6 `506–753` 248 · R8 `769–869` 101 · R18 `1364–1723` 360 · R24 `2031–2082` 52 · R31 `3264–3288` 25) → `static/src/39b-dashboard.js` (**1.143** com cabeçalho), que entra **ANTES** do 40 (como 37/38/39) | **2.219** | **0** — o primeiro domínio extraído sem NENHUM statement de topo: 40 declarações (31 funções + 9 de estado), todas literais |
| **10b** | *(follow-up de cobertura)* Dashboard / KPI cards | — (nenhum código movido) | ✅ **#568** — spec dedicado `dashboard-kpi-cards.spec.js` com snapshot determinístico (`page.route('**/api/snapshot*')` + `route.fetch()` e override, `/api/stream` abortado e a guarda de SW da #562): os 4 cells passam a ter asserção **incondicional** (`#kpi-hashrate`=`2.00 TH/s`, `#kpi-bestdiff`=`5.00 M`, `#kpi-poolhr`=`3.00 TH/s`, `#kpi-shares`=`42/h`) + o fallback do share rate (`1234 total`). A mutação que **sobreviveu** ao PR 10 — desligar a escrita de `#kpi-hashrate` em `renderKpiCards` (R31) — agora **derruba o spec (4/4, exit 1)** | **2.219** (inalterado) | — |
| — | **residual** — Docs, Settings, Export, Braiins buy, AI Chat/Operator, Theme, primitivas DOM, shell (sidebar/módulos), `boot()` | R2+R4+R14+R15+R17+R19+R21+R25+R27+R28+R29+R30 | — | **2.219** | — |
...
**A meta de ~4.000 linhas é atingida no PR 9** — **executado**: com os PRs 6, 7 e 8 concluídos o god file parava em **4.320**, e o PR 9 (1.032 linhas movidas) o levou a **3.292**\*, abaixo da meta de ~4.000. **A meta está cumprida.**

**\*Por que 3.292 e não a projeção 3.288 (reconciliado na Issue #557 · PR #558).** A projeção contava só as linhas movidas — 4.320 − 1.032 = 3.288 — e ignorava as **4 linhas de costura** deixadas nos dois pontos de corte: as 2 linhas de comentário `// → domínios Wallet crypto (R3) e Support (R20) extraídos para …` (uma por corte) e 2 linhas em branco. O diff medido em `static/src/40-app-logic.js` entre `3336c34` (pré-PR 9) e `975172e` (HEAD) é **1.030 removidas / 2 adicionadas** — o `diff` alinha 2 das linhas em branco removidas com as 2 adicionadas —, líquido **−1.028** → **4.320 → 3.292**. A costura é **byte-exata**: removendo as 2 linhas de comentário (e a linha em branco seguinte a cada uma) e reinjetando o **corpo verbatim** do `37-wallet-support.js` (linhas 31–1.062 = 1.032 linhas) nas duas posições, o god file pré-PR 9 é reproduzido **byte a byte** (`4.321 = 4.321` no split por `\n`). Nenhuma linha de código foi criada, perdida ou reescrita.

O residual ainda tem domínios extraíveis — e o **PR 10 natural de Dashboard/`render()`** (R5+R6+R8+R18+R24+R31) foi executado **fora da meta**, medido em **1.082 linhas** (a projeção era ~1.096; ver o achado abaixo). Os demais continuam **fora da meta**; a lista acima é o que fecha o objetivo.
...
**Duas lacunas de cobertura registradas pelo PR 7 — e fechadas na Issue #548 (PR follow-up).** (1) Uma mutação no *math* (`simulateDifficultyShift` retornando `base`) **sobrevivia**: `probability-whatif.spec.js` pulava a comparação numérica quando o servidor não tem dados de pool (guard `hasData`, com `#bh-whatif-diff` = '—'), e `base` coincide com o resultado correto com o slider em 0. (2) A SUITE 33 do harness JS **espelhava** o what-if em vez de carregar o fragmento — o mesmo padrão que escondeu 3 defeitos reais no PR 1.

A #548 fecha as duas. No harness, `loadFragment('42-probability.js', …, { window: {} })` passa a fornecer **`simulateDifficultyShift` e `_bhFinitePositive` reais** para a SUITE 33 (o espelho à mão foi removido), aproveitando o sandbox mínimo introduzido no PR 2 (#515) — o fragmento tem 1 statement de topo (`window.setProfitMode = …`). No e2e, o spec ganhou `forceBlockHuntData()`: intercepta `/api/snapshot` e injeta `network.difficulty = 110 T` + `worker.bestDifficulty = 10 G`, corta `/api/stream` e compara os 4 valores **numéricos** do readout (9.09e-3% / 8.26e-3% / 6.99e-3% / 1.21e-2%) em vez de só o sinal.

> **Achado do follow-up — o service worker era a causa raiz, não o `hasData`.** A primeira tentativa (`page.route('**/sw.js*')`) **não** funcionou: o boot registra o SW, ele assume o controle, o app faz `location.reload()` e — pior — o **próprio SW passa a responder `/api/snapshot` do cache**. `page.route` não intercepta request originado de dentro do SW, então o fixture era silenciosamente ignorado e o painel voltava ao snapshot real (`0.00e+0%`, dificuldade 127 T). A correção é negar o registro via `page.addInitScript` (`navigator.serviceWorker.register = () => Promise.reject(...)`): sem SW não há reload nem cache, e o poll interceptado vira a única fonte. Fica registrado porque **qualquer** e2e futuro que dependa de `page.route` em `/api/*` desta app precisa da mesma guarda.

**Fechado na Issue #562 — a guarda vira infra compartilhada.** Em vez de cópia inline, a guarda passou a ser `tests/e2e/support/sw-guard.js` (`denyServiceWorker(page)`), aplicada também aos **2 specs que mockavam `/api/*` sem guarda alguma**: `auth.spec.js` (5 `page.route` em `/api/auth/*` + `/api/axe-fleet/*`) e `live-metrics-pagination.spec.js` (patch de `window.fetch` servindo `/api/snapshot` + `/api/leaderboard`); o `probability-whatif.spec.js` passou a usá-la no lugar do bloco duplicado. A varredura da suíte mostrou que os outros **13** specs que mockam `/api/*` já neutralizavam o SW via `test.use({ serviceWorkers: 'block' })` — o nativo do Playwright, equivalente à guarda. Prova medida (probe com/sem guarda, chromium + mobile-chrome): **sem** guarda `controller: true, registrations: 1, caches: ['cypher65-v12']`; **com** guarda `controller: false, registrations: 0, caches: []`. Honestidade: os 2 specs corrigidos **passavam mesmo sem** a guarda — a mudança é **preventiva** (elimina o reload do `controllerchange` e o bypass do `page.route` pelo SW), não o fix de uma falha observada.

Prova de mutação após a #548: mutar `simulateDifficultyShift` no fragmento derruba a SUITE 33 (**5 asserções**) e mutar o mesmo ponto no **bundle** `static/app.js` (o artefato que o navegador realmente executa) derruba o spec (**4/4**). Antes da #548, só a primeira dessas duas provas existia.

**Achado do PR 10 (#561) — o primeiro domínio sem NENHUM statement de topo, e a posição forçada pelo `boot()`.** O cluster Dashboard/`render()` (R5+R6+R8+R18+R24+R31) mede **1.082 linhas** em 6 recortes, somando **40 declarações (31 funções + 9 de estado)** e **zero statements de topo** — o primeiro domínio extraído sem nenhum (nem `window.X = …`, nem listener). E ele **não pôde** ir depois do 40: o `boot()` é chamado no topo do god file e o **prefixo síncrono** dele toca o domínio — `initCharts()` (1ª instrução) escreve/lê `const charts`, e `await fetchSnapshot()` chama `fetchSnapshot()`, que antes do primeiro `await` lê e escreve `let _snapshotFetching`. **Prova A/B medida (probe e2e, chromium + mobile-chrome):** com `39b-dashboard.js` **depois** do `40-app-logic.js` no MANIFEST o boot lança `Cannot access 'charts' before initialization`; **antes**, 0 pageerrors. Logo o fragmento entra como `39b` — entre o 39 e o 40.

**Prova de permutação do PR 10 (medida pelo `git diff -U0`, não pelas anotações do autor).** O corpo do `39b-dashboard.js` é um recorte **contíguo e byte-idêntico** de **1.082 linhas** do bloco removido do god file: as 11 linhas adicionadas ao god file são só as **6 costuras** (`// → … extraído para …`) **+ 5 linhas de uma edição de doc** (o comentário de fecho do IIFE, que citava `renderKpiCards`, reescrito no lugar) — **zero** linhas reescritas, **zero** em branco adicionadas. No bundle (`static/app.js`), a comparação por *multiset* mostra que **toda** diferença é comentário: **nenhuma linha executável foi criada, perdida ou alterada** (a ordem muda por construção — o bloco passou a ser avaliado antes do 40 — e todas as `function` do IIFE sofrem hoisting). God file: **3.292 → 2.219**. *(Duas correções de honestidade contra as anotações iniciais: a projeção/prosa dizia 1.094 linhas e listava o intervalo R31 como `3265–3288`/24; o medido é **1.082** e `3264–3288`/25 — os números do cabeçalho do fragmento foram reescritos para os medidos.)*

**Achado de cobertura do PR 10 — duas mutações SOBREVIVERAM, e isso fica registrado.** (a) Desligar a escrita de `#kpi-hashrate` dentro de `renderKpiCards` **não** derruba `dashboard.spec.js`, porque a asserção que pegaria isso (`#kpi-hashrate` ≠ `—`) é **condicional** à presença de um worker conectado (`#hud-hashrate` visível) e o servidor local de e2e sobe sem worker. ✅ **Fechado pela #568** (linha **10b**): o spec dedicado injeta o fixture e a MESMA mutação hoje **derruba `dashboard-kpi-cards.spec.js` 4/4 com exit 1** — a célula fica presa no placeholder `—` e o `waitForFunction` estoura. (b) Comentar a chamada `renderOperationalOverview(…)` **dentro do `render()`** não derruba `operational-overview.spec.js`, porque o `49-axe-fleet.js` **também** chama `renderOperationalOverview` (em `fetchAxeFleet`), e é por esse caminho que o spec é alimentado. A prova de mutação válida acabou sendo no **modelo compartilhado**: desligar o ramo `CRITICAL` de `buildOperationalOverviewModel` (R5) derruba `operational-overview.spec.js` (**2 falhas** — `ATTENTION` onde se espera `CRITICAL`).

**Gates do PR 10:** permutação 1.082 = 1.082 ✅ · bundle só-comentários ✅ · probe A/B de posicionamento ✅ · mutação R5 detectada (2 falhas) ✅ · JS core **1.435** ✅ · pytest **3.366** ✅ · `check:frontend` verde ✅ · e2e **82 testes** verdes (dashboard 58 + operational-overview/live-metrics-pagination/probability-whatif/wallet-identity/market-affiliate 24) ✅.
...
**Achado do PR 9 (#551) — o caso inverso do PR 8, e a régua é sobre ORDEM, não sobre TDZ.** Aqui o fragmento também precisa vir **antes** do 40, mas por um motivo diferente de todos os anteriores: não há TDZ nenhum. As **18 statements de topo** do domínio (`renderSupportMethods()`, `loadDonations()`, `toggleWalletCTA()`, os 4 × `QrPoly.prototype.*`, o IIFE `buildQrMath` e os 9 listeners de wallet) executam durante a avaliação do fragmento, e **no arquivo original elas rodavam em 2711–2928, ou seja ANTES do `boot();` da linha 3260**. Com o fragmento *depois* do 40, `renderSupportMethods()`/`loadDonations()`/o listener do `_onSupportOpen` rodariam **depois** do boot — inversão de ordem silenciosa, sem erro no console e sem teste que a pegue. Todas as dependências dessas statements resolvem por **hoisting** (`escapeHtml`, `cssVar`, `openModalAnimated`, `authFetch` são `function` declarations do IIFE único — o `authFetch` mora no `38-billing-auth.js`, que vem *depois*, e isso é seguro) ou por fragmento anterior (`dom`, `const` no `20-dom-primitives.js`). A lição a registrar: **a régua do §3.3 é sobre a posição relativa das execuções de topo, e "antes do 40" pode ser exigido tanto por TDZ quanto por ordem.**

Verificações do PR 9: 2 blocos contíguos (R3 `44–642` = 599 e R20 `2599–3031` = 433, somando exatamente a projeção de 1.032); 16 nomes de estado com **zero** colisão com os outros 14 fragmentos e **zero** leitura externa; nenhum statement em coluna 0 acrescentado (o `dom.walletSave?.addEventListener` da linha 2928 **já** estava em coluna 0 e foi preservado assim, de propósito, para não inflar o diff); e nenhum chamador de nível de módulo fora das faixas — os usos em `38-billing-auth.js` (`qrEncode`/`qrSvg`/`detectWebLN`) e no 40 (`toggleWalletCTA()` dentro do `render()`, `renderSupportMethods()` dentro do `activateModule`, `openWalletModal`/`closeWalletModal` no handler de teclado) são todos runtime.

**Prova de mutação dupla (2 caminhos distintos):** mutar o *listener de topo* do `_onSupportOpen` (`'#support-expand-btn'` → id inexistente) derruba o `webln.spec.js` **10/26** — prova que a statement de topo relocada **executa** na posição nova; mutar o *opening tag* do `qrSvg` derruba o `upgrade-btc.spec.js` **6/18** — prova que a ponte com o `38-billing-auth.js` (que consome `qrEncode`/`qrSvg` do fragmento novo) segue viva por hoisting. Ambos restaurados.

**Achado de cobertura registrado pelo PR 9 — FECHADO na Issue #553 (PR #554).** O harness `tests/test_app_js_core.js` ainda **espelha** `validateBitcoinAddress` (linha 191) e `QrPoly` (linha 3892) — os dois acabaram de **mover** para `37-wallet-support.js` e ambos são marcados no fonte com o comentário "Pure helpers mirrored in tests". É o mesmo padrão que o #515 resolveu para o Market e o #548 para o Block Hunt; a correção é barata (`loadFragment('37-wallet-support.js')` nas duas suítes) mas ficou **fora do escopo** do PR 9, como follow-up explícito em vez de silêncio.

O follow-up foi **executado** na Issue #553 (PR #554, mergeado em `975172e`): o harness passou a carregar o **fragmento real** — `loadFragment('37-wallet-support.js', …, sandbox)`, com um sandbox mínimo que mantém listeners e requests de topo inertes (sem DOM, rede, pagamento ou efeitos assíncronos) — e o espelho saiu com **−441 / +35 linhas** no `tests/test_app_js_core.js` (as linhas 191/3.892 citadas acima eram o estado pré-#554). Prova de mutação no código de produção: `validateBitcoinAddress` forçado a rejeitar tudo → **7/16 falhas, exit 1**; `qrEncode` forçado a devolver matriz vazia → **37 falhas, exit 1**. **Achado colateral do follow-up, e o mais importante dele:** os blocos legados `[btc-valid]` e `[chunk]` imprimiam `FAIL` mas **não incrementavam o contador global** — a suíte terminava com **exit 0 mesmo com asserts quebrados**; ambos agora delegam ao `assertEqual`. JS core: **1.406 → 1.428** testes. O antecessor `QrPoly` deixou de existir no harness porque o núcleo QR inteiro (`buildQrMath`/`QrPoly`/`qrEncode`/`qrSvg`) é carregado do fragmento.

**Duas lacunas registradas na Issue #557 · PR #558 — a primeira FECHADA na Issue #559, a segunda ainda aberta.** Elas não são o mesmo caso.

**(1) Helpers de auth — FECHADA na Issue #559.** `authBuildHeaders`/`authIsExpired`/`authSessionValid` (Suite 22, ~linhas 2.594–2.631 do harness) eram espelhos de código que **já morava** no `38-billing-auth.js`, que o harness **não** carregava; como esse fragmento é lido pelo prefixo **síncrono** do `boot()`, era o candidato mais relevante ao mesmo tratamento do #515/#548/#553. Foi feito: o harness carrega o fragmento via `loadFragment('38-billing-auth.js', …, { window, localStorage })` (o fragmento tem 1 statement de topo — `window.openUpgradeModal = …` — e o `localStorage` mínimo é o que o `licenseKey()` real consulta), o espelho saiu e as **18 asserções originais seguem rodando contra o fonte** (com `_cypher65_license` vazio o fonte se comporta como o espelho; a suíte prova isso com uma asserção de pré-condição).

O espelho **não era fiel** — e a divergência era de comportamento, não de estilo: ele omitia `licenseKey()`, então `authBuildHeaders(null)` era afirmado como `{}` enquanto o fonte real devolve `{ 'X-License-Key': … }` quando há licença PRO persistida. Ou seja, a asserção `'no token -> empty headers'` codificava o contrato **do espelho**, e o header `X-License-Key` (tier R1/PRO) não era exercitado por nenhum teste. As 7 asserções novas cobrem exatamente esse caminho (licença + token, licença sem token, valor preservado sem trim, licença vazia ignorada, limpeza). **Prova de mutação, e ela é a mais direta do padrão:** desligar o branch do `X-License-Key` no fragmento — isto é, restaurar *o contrato do espelho antigo* no código de produção — derruba a suíte com **3/1.435 falhas e exit 1**; esse mutante **sobrevivia** ao harness anterior. Zerar a margem de 30s do `authIsExpired` derruba 1 asserção (prova de que as asserções antigas também passaram a exercer o fonte). JS core: **1.428 → 1.435**.

**(2) `acctRankLabels` (Suite 27) — PERMANECE aberta.** Este é **estrutural**, não dívida de método: a função ainda vive no `40-app-logic.js`, e o harness **não pode** carregar o god file (o `boot();` do topo rodaria). O espelho só sai quando o domínio (R7/R6) for extraído para um fragmento — o que é exatamente o tipo de ganho colateral que a decomposição traz.

**Alvo do frontend (fixado em §3.3, 2026-09-12; PR 9 executado).** `40-app-logic.js` saiu de **7.706** para **3.292 linhas** ao fim do **PR 9** — a meta de ~4.000 foi atingida (o PR 8 parou em 4.320). O que sobra são ~3.300 linhas de **orquestração**: `boot()`, o loop de poll, o sistema de módulos/sidebar, `render()` e as primitivas de DOM — mais os blocos de Dashboard/Docs/Settings que ainda são extraíveis num PR 10+, fora desta meta.

Estado de execução:

| Trilha | Entregue | Restante até a meta |
|---|---|---|
| Frontend | 1, 1b, 2, 2b, 3, 4a, 4b (+ limpeza #525), 5 (#529), 6 (#540), 7 (#542), 7b (#548), 8 (#545), 8b (#559), 9 (#551), 9b (#553) e 10 (#561, *fora da meta*), 10b (#568) | — (**meta atingida**) |
