...
def test_global_settings_untouched_by_tenant_writes():
    ...
    _settings_mod.save_setting("webhook_url", "https://evil.example/hook",
                               tenant_id="tenant-aaa")
    assert _settings_mod.load_settings()["webhook_url"] == "https://operator.example/hook"
    assert _settings_mod.load_settings("tenant-aaa")["webhook_url"] == "https://evil.example/hook"
...
def test_braiins_credentials_tenant_uses_own_key(monkeypatch):
    ...
    _settings_mod.save_setting("braiins_api_key", "tenant-key",
                               tenant_id="tenant-aaa")

    assert _tools_mod.braiins_credentials(tenant_id="tenant-aaa")["api_key"] == "tenant-key"
...
def test_settings_post_writes_tenant_row(rclient):
    ...
    resp = rclient.post("/api/settings",
                        headers={"Authorization": "Bearer " + _token("tenant-aaa")},
...
def test_config_backup_is_tenant_scoped(rclient, monkeypatch):
    ...
    resp = rclient.get("/api/config/backup",
                       headers={"Authorization": "Bearer " + _token("tenant-aaa")})
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_test_braiins_route_uses_tenant_key(rclient, monkeypatch):
    ...
    monkeypatch.setattr(
        _app_module._rental_perf, "fetch_braiins_contracts", _capture)
