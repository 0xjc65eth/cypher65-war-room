...
class TestRateLimitKey:
    ...
    def test_invalid_token_falls_back_to_ip(self, app_ctx):
        with app_ctx.app.test_request_context(
            "/api/snapshot", headers={"Authorization": "Bearer not-a-real-token"}
        ):
            key = app_ctx._rate_limit_key()
            assert key.startswith("ip:")
...
class TestTokenSubCache:
    ...
    def test_cache_returns_same_tenant_without_verify(self, app_ctx, monkeypatch):
        monkeypatch.setenv(
            "SECRET_KEY", "cache-test-secret-0123456789-abcdef0123456789"
        )
        calls = {"n": 0}
        real_verify = None
...
class TestPollWait:
    def test_jitter_desyncs_workers(self):
        ...
        assert all(
            POLL_INTERVAL <= w < POLL_INTERVAL + POLL_JITTER_MAX + 1 for w in waits
        )
        # …and they are NOT all identical (thundering-herd fix).
        assert len(waits) > 1

    def test_backoff_grows_and_caps(self):
        from services.user_polling import _poll_wait, POLL_INTERVAL, POLL_MAX_BACKOFF
    ...
    def test_global_cache_bounded_eviction(self, monkeypatch):
        ...
        monkeypatch.setattr(sa, "_GLOBAL_CACHE_MAX", 50)
        monkeypatch.setattr(sa, "_global_cache", {})
        for i in range(200):
            sa._update_global(f"user_bc1q{i}", {"workerData": []})
        assert len(sa._global_cache) <= 50
        # Oldest evicted first; the most recent entries survive.
        assert "user_bc1q0" not in sa._global_cache
        assert "user_bc1q199" in sa._global_cache
